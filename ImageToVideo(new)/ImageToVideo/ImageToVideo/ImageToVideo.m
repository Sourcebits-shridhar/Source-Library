...
@interface ImageToVideo(private)

+ (CVPixelBufferRef) pixelBufferFromCGImage:(CGImageRef)image size:(CGSize)size;
+ (UIImage *)imageWithImage:(UIImage *)image scaledToSize:(CGSize)newSize withContentMode:(eContentMode)contentmode;
...
@synthesize customTransitionSmoothness = mCustomTransitionSmoothness;

@synthesize transitionEffect = mTransitionEffect;
@synthesize transitionSmoothness = mTransitionSmoothness;
@synthesize contentmode = mContentmode;
...
- (id)init
{
    ...
    if (self)
    {
        ...
        self.repeat = YES;
        self.customTransitionSmoothness = 0;
    }
    return self;
}
...
- (void)prepareVideo:(NSString *)name withImages:(NSMutableArray *)images andAudio:(NSURL *)audio
{
    ...
    [[NSFileManager defaultManager] createDirectoryAtPath: [NSString stringWithFormat:@"%@/Video",documents]
    ...
    for (UIImage *image in images)
    {
        ...
        UIImage *resizedImage = [ImageToVideo imageWithImage:image 
                                                  scaledToSize:CGSizeMake(kVideoWidth, kVideoHeight) withContentMode:self.contentmode];
        
        CGImageRef img =  [resizedImage CGImage];
        
        mBuffer = nil;
        mBuffer = [ImageToVideo pixelBufferFromCGImage:img size:CGSizeMake(kVideoWidth, kVideoHeight)];
        ...
        int frameRate = 0;
        if (self.customTransitionSmoothness != 0 && self.transitionSmoothness == custom)
        {
            frameRate = self.customTransitionSmoothness;
        }
        if (self.transitionSmoothness == high)
        {
            frameRate = 35;
        }
        else if (self.transitionSmoothness == medium)
        {
            frameRate = 25;
        }
        else if (self.transitionSmoothness == low)
        {
            frameRate = 15;
        }
        ...
        if (self.transitionEffect == eShrink)
        {
            for (int frames = 1; frames <= frameRate; frames++)
            {
                ...
                [ImageToVideo imageWithImage:resizedImage 
                                  scaledToSize:CGSizeMake(conversionWidth, conversionHeight) withContentMode:self.contentmode];
                CVPixelBufferRef conversionBuffer = nil;
                if (indx < [images count] - 2)
                {
                    
                    UIImage *backgroundImage = [images objectAtIndex:indx + 1];
                    
                    UIImage *resizedBackground = [ImageToVideo imageWithImage:backgroundImage 
                                                                   scaledToSize:CGSizeMake(kVideoWidth, kVideoHeight) withContentMode:self.contentmode];
                    UIImage *appendedImage = [ImageToVideo appendImage:resizedBackground
                                                                      to:conversionImage 
                    ...
                    conversionBuffer = [ImageToVideo pixelBufferFromCGImage:[appendedImage CGImage] size:CGSizeMake(kVideoWidth, kVideoHeight)];
                }
                else if (indx == [images count] - 2)
                {
                    UIImage *appendedImage = [ImageToVideo appendImage:nil
                                                                      to:conversionImage 
                                                                 atPoint:CGPointMake((kVideoWidth - conversionImage.size.width)/2,
                                                                                     (kVideoHeight - conversionImage.size.height)/2)
                                                              otherPoint:CGPointZero];
                ...
                {
                    conversionBuffer = [ImageToVideo pixelBufferFromCGImage:[conversionImage CGImage] size:CGSizeMake(kVideoWidth, kVideoHeight)];
                }
                while (adaptor.assetWriterInput.readyForMoreMediaData==NO)
                {
                    [NSThread sleepForTimeInterval:0.1];
        ...
        else if (self.transitionEffect == eDissolve)
        {
            for (int frames = 1; frames <= frameRate; frames++)
            {
                ...
                NSLog(@"This is the Alpha -> %f",alpha);
                ...
                if (indx < [images count] - 2)
                {
                    ...
                    mBuffer = [ImageToVideo pixelBufferFromCGImage:[appendedImage CGImage] size:CGSizeMake(kVideoWidth, kVideoHeight)];
                }
                else
                {
                    mBuffer = [ImageToVideo pixelBufferFromCGImage:[alphaImage CGImage] size:CGSizeMake(kVideoWidth, kVideoHeight)];
                }
        ...
        if (self.transitionEffect == eCrush)
        {
            for (int frames = 1; frames <= frameRate; frames++)
            {
                ...
                UIImage *conversionImage = 
                [ImageToVideo imageWithImage:resizedImage 
                                  scaledToSize:CGSizeMake(conversionWidth, kVideoHeight) withContentMode:self.contentmode];
                CVPixelBufferRef conversionBuffer = nil;
                if (indx < [images count] - 2)
                {
                    UIImage *backgroundImage = [images  objectAtIndex:indx + 1];
...
- (void)addAudioToVideo
{
    if (self.cancel == NO)
    {
        ...
        __block ImageToVideo *temp = self;
...
+ (UIImage *)imageWithImage:(UIImage *)image scaledToSize:(CGSize)newSize withContentMode:(eContentMode)contentmode
{
    ...
    UIImage *convertedImage = [UIImage imageWithData:UIImageJPEGRepresentation(image, 1.0f)];
    
    CGRect drawRect = CGRectMake(0, 0,newSize.width, newSize.height);
    
    
    CGContextSetRGBFillColor(UIGraphicsGetCurrentContext(), 0.0f/255.0f, 0.0f/255.0f, 0.0f/255.0f, 1.0f);
    ...
    else
    {
        ...
        if (heightSize > newSize.height)
        {
            ...
            float oldHeight = heightSize;
            heightSize = newSize.height;    
            widthSize = widthSize/((float)oldHeight/newSize.height);
        }
        
        xCoordinate = (newSize.width - widthSize)/2;
        yCoordinate = (newSize.height - heightSize)/2;
    }
    
    NSLog(@"Converted size =  {%.02f, %.02f}",widthSize, heightSize);
    
    [convertedImage drawInRect:CGRectMake(xCoordinate, yCoordinate, widthSize, heightSize)];
